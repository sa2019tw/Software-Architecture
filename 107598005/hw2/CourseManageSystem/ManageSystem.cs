...
namespace CourseManageSystem
{
    public partial class ManageSystem : Form
    {
        ...
        private void txtEdit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                string[] textCheck = txtEdit.Text.Split(new char[]{' '}, StringSplitOptions.RemoveEmptyEntries);
                if(txtEdit.Text == "" || textCheck.Length == 0)
                {
                    MessageBox.Show("課程名稱不能為空", "警告");
                }
                else
                {
        ...
        {
            string[] textCheck = nameBox.Text.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            if (nameBox.Text == "" || textCheck.Length == 0)
            {
                MessageBox.Show("課程名稱不能為空", "警告");
            }
            else
